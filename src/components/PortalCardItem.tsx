...
export default function PortalCardItem({ card, isFavorite, onToggleFavorite }: PortalCardItemProps) {
      ...
      <div className={`relative overflow-hidden aspect-video ${!imgLoaded || imgFailed ? `bg-gradient-to-br ${gradient}` : ''}`}>
        ...
        <div className="absolute top-1.5 right-1.5 z-10">
          <button
            ...
            className="w-5.5 h-5.5 rounded-full flex items-center justify-center backdrop-blur-sm border transition-all hover:scale-110"
            style={{
          ...
          >
            <svg className="w-3 h-3" viewBox="0 0 24 24" fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={isFavorite ? 0 : 2}>
